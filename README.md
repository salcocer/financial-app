...
> The working brand name is **"Finly"** (`constants/auth.ts`) — `app.json` still carries the default Expo template name (`HelloWorld` / `expo-template-default`) and hasn't been renamed yet.
...
- `app.json` still has the default Expo template `name`/`slug` (`HelloWorld` / `expo-template-default`) rather than `Finly`.
